...
namespace Microsoft.Azure.Cosmos.Table.Queryable
{
	internal static class ReflectionUtil
	{
		...
		internal static MethodInfo DictionaryGetItemMethodInfo
		{
			get;
			set;
		}

		internal static MethodInfo TableQueryProviderReturnSingletonMethodInfo
		{
			get;
			set;
		}

		internal static MethodInfo ProjectMethodInfo
		{
			get;
			set;
		}
		...
		static ReflectionUtil()
		{
			...
			foreach (MethodInfo allLinqOperator in GetAllLinqOperators())
			{
				...
				if (dictionary.TryGetValue(canonicalMethodDescription, out SequenceMethod value))
				{
					StaticMethodMap.Add(allLinqOperator, value);
					StaticInverseMap[value] = allLinqOperator;
				}
			}
			DictionaryGetItemMethodInfo = typeof(IDictionary<string, EntityProperty>).GetMethod("get_Item");
			TableQueryProviderReturnSingletonMethodInfo = typeof(TableQueryProvider).GetMethod("ReturnSingleton", BindingFlags.Instance | BindingFlags.NonPublic);
			ProjectMethodInfo = typeof(TableQuery).GetMethods(BindingFlags.Static | BindingFlags.Public).First((MethodInfo m) => m.Name == "Project");
		}
		...
		{
			if (TryIdentifySequenceMethod(method, out SequenceMethod sequenceMethod2))
			{
				return sequenceMethod2 == sequenceMethod;
			}
			return false;
		}
		...
		{
			if (genericArgumentOrdinals != null && genericArgumentOrdinals.TryGetValue(type, out int value))
			{
		...
		{
			return typeof(System.Linq.Queryable).GetMethods(BindingFlags.Static | BindingFlags.Public).Concat(typeof(Enumerable).GetMethods(BindingFlags.Static | BindingFlags.Public));
		}
	}
}
